...
.f2 <- function(x, ga,g) {
  ...
  for (i in unique(g)){
    if(i!=0){
      index = which(g == i)
      result[index,index]=max((1-(ga/as.numeric(norm(result2[index,index], 'F')))), 0) * result2[index,index]
    }
  }
  results=x-result
  return(results)
...
.g2 <- function(x, a, lambda,g) {
  ...
  for (i in unique(g)){
    if(i!=0){
      ...
      result[index,index]=a[index,index]+(min((lambda/as.numeric(norm(B[index,index], 'F'))), 1) * B[index,index])
    }
  }
...
.proximal <- function(a, g, lambda, epsilon, gamma, rho, W, iterMax) {
  ...
  for (i in 1:iterMax) {
    ...
    x1=Ad%*%y1
    prox1=.f1(x1, (4 * lambda * gamma*W))

    p1 = y1 - t(Ad)%*%prox1


    x2=Atot%*%y2
    prox2 = .g1(x2, a, (lambda*W))
    p2 = y2 - 2*t(Atot)%*%prox2
    x3=Ag%*%y3
    prox3 = .f2(x3,4*gamma*lambda*epsilon,g)
    p3 = y3 - t(Ag)%*%prox3

    x4=Atot%*%y4
    prox4 = .g2(x4, a, (lambda*epsilon),g)
...
precompute_backmap <-function(C,D,covType = "cov", intertwined=FALSE,thre = "soft"){
...
kdiffnet <- function(C,D, W, g=rep(1,100), epsilon = 1, lambda = 0.05, knowledgeType="EV",gamma=4.0,covType = "cov", intertwined=FALSE,thre = "soft",rho=0.05,iterMax=20, B=0, precompute=FALSE){
  if(precompute==FALSE){
  covX = compute_cov(C,covType)
  covY = compute_cov(D,covType)
